...
// ---------------------------------------------------------------------
// Aide pour lire les paramètres envoyés par le casino
// ---------------------------------------------------------------------
// Le casino peut nommer ses variables différemment selon la plateforme.
// On essaie plusieurs noms courants, dans cet ordre de priorité.
function readCode(q) {
  return q.sub || q.subid || q.sub_id || q.s2 || q.affsub || q.aff_sub || q.click_sub || null;
}
function readAmount(q) {
  const raw = q.amount ?? q.sum ?? q.value ?? q.deposit_amount ?? q.revenue ?? q.ngr ?? "0";
  const n = Number(raw);
